<template>
  <div class="w-full max-w-7xl mx-auto">
    <h1 class="text-2xl font-bold mb-4">Crear Nueva Nota</h1>
    <FormKit
      type="form"
      @submit="handleSubmit"
      :submit-attrs="{
        ...
        text: loading ? 'Creando...' : 'Crear Nota',
        disabled: loading
      }"
    >
      <FormKit
...
const handleSubmit = async () => {
  loading.value = true;
  try {
    const response = await noteStore.createNote(note.value);
    await Swal.fire({
      icon: 'success',
      title: '¡Creado!',
    ...
    await Swal.fire({
      icon: 'error',
      title: 'Error',
      text: error.response?.data.message || 'No se pudo crear la nota. Inténtalo de nuevo.',
    });
  } finally {
    loading.value = false;
  }
